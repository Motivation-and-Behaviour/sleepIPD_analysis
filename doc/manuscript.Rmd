...
tab <- demog_table |> 
  dplyr::select(Variable = level, Value = out) |> 
  split(demog_table$name)

tab <- append(tab["Numeric variables"], tab[!names(tab) %in% "Numeric variables"])

# Reorder socioeconomic status
tab$`Socioeconomic Status`$Variable <- factor(tab$`Socioeconomic Status`$Variable, levels = c("Low","Medium","High"))
tab$`Sleep Conditions Reported` <- tab$`Sleep Conditions Reported`[tab$`Sleep Conditions Reported`$Variable == "Yes"]
tab$`Socioeconomic Status` <- tab$`Socioeconomic Status`[order(tab$`Socioeconomic Status`$Variable),]

papaja::apa_table(tab, caption = "Participant characteristics",
